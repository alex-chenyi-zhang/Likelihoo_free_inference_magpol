...
int main(int argc, char* argv[]){
    ...
    double delta_w = 0.1; // to pick trial weight in the MCMC choose w_trial = w_old + delta_weight*(uni_01(mer_twist)*2-1)
    ...
    config_file.close();
    std::cout << "Number of data points: " << n_data_samples << "\n";
    
    double ** ss_data = new double *[n_data_samples];
    for (int i_ss = 0; i_ss < n_data_samples; i_ss++){
        ss_data[i_ss] = new double[3];
    }
    ...
    if (config_file.is_open()){
        ...
        while(std::getline(config_file, line)){
            std::stringstream ss(line);
            if (!(ss >> ss_data[i_ss][0] >> ss_data[i_ss][1] >> ss_data[i_ss][2])){ break; }
            i_ss++;
        }
    }
    else{
    ...
    syn_like = 0;
    for (int i_ss = 0; i_ss < n_data_samples; i_ss++){
        syn_like += log_synth_likelihood(inverse_covariance_matrix, ss_mean_estimates[0], ss_data[i_ss], ss_abs_covariance_determinant[0]); 
    }
    ...
    for (int i_param = 1; i_param < n_param_samples; i_param++){
        ...
        trial_syn_like = 0;
        for (int i_ss = 0; i_ss < n_data_samples; i_ss++){
            trial_syn_like += log_synth_likelihood(inverse_covariance_matrix, ss_mean_estimates[i_param], ss_data[i_ss], ss_abs_covariance_determinant[i_param]); 
        }
