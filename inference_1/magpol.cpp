...
saw_MC::saw_MC(int x, int y, int z, int s_lag, float j_spins, float alpha, float inv_T, std::string f1, std::string f2, std::string f3, bool compute_ss) : uni_R(0.0, 1.0), uni_I_poly(1,x-1), uni_G(0,47-1), uni_spins(-1,1), 
    ...
    n_swaps = z;
    samples_lag = s_lag;
    n_samples = y*z/samples_lag;
    if ((y*z)%samples_lag != 0){ n_samples +=1;}
...
void saw_MC::initialize_configuration(bool from_files){
    ...
    // Initialize to rigid rod with random spin values
        ...
        for (int i = 0; i < n_mono; i++){
            ...
            spins[i] = uni_spins2(mt); //This ways spins are either 1 or 0
...
void saw_MC::initialize_parameters(float weight){
    ...
    if (config_file.is_open()){
        ...
        while(std::getline(config_file, line)){
            std::stringstream ss(line);
            if (!(ss >> CpG_fraction[i_mono])){ break; }
            i_mono++;
        }
    }
    else{
    ...
    for (int i_mono = 0; i_mono < n_mono; i_mono++){ h_fields[i_mono] = CpG_fraction[i_mono]*weight;}
}
...
/*********************************************************************************************/
float saw_MC::lag_p_spin_autocovariance(int p){ // p is the lag used to calculate the autocovariance of the spins along the chain
    float avg = 0;
    float avg_lag = 0;
    float var = 0;
    float var_lag = 0;
    float corr = 0;
    for (int i_mono = 0; i_mono < n_mono-p; i_mono++){
        avg += spins[i_mono];
        avg_lag += spins[i_mono+p];
        corr += spins[i_mono]*spins[i_mono+p];
    }
    avg = avg/(n_mono-p);
    avg_lag = avg_lag/(n_mono-p);
    corr = corr/(n_mono-p);
    for (int i_mono = 0; i_mono < n_mono-p; i_mono++){
        var += (spins[i_mono]-avg)*(spins[i_mono]-avg);
        var_lag += (spins[i_mono+p]-avg_lag)*(spins[i_mono+p]-avg_lag);
    }
    var = var/(n_mono-p-1);       // the minus 1 is to get an unbiased estimator of the variance. but is it needed thou?
    var_lag = var_lag/(n_mono-p-1);
...
void saw_MC::spins_MC(){
    ...
    for(int i_flip = 0; i_flip < n_flips; i_flip++){
        ...
        if(spins[flip_candidate]==0){ trial_spin_value = 1;}
        else{ trial_spin_value = 0;}
...
multiple_markov_chains::multiple_markov_chains(std::string input_filename): mer_twist(rand_dev()){
    ...
    if (file_in.is_open()){
        ...
        while(std::getline(file_in, line)){
            ...
            if (a=="conf_file")   
                ...
                tmp >> spins_filename;
            else if (a=="features_file")
                tmp >> features_filename;
            else if (a=="initialize_from_files")
                tmp >> from_files;
            else if (a=="initialize_weight")
                tmp >> initialize_weight;
            else if (a=="weight")
                tmp >> regression_weight;
    ...
    if (!initialize_weight){ regression_weight = weight_distribution(mer_twist); }
    std::cout << "Weight value: " << regression_weight << "\n";
